...
def clf_params_non_interactive(interactive):
    ...
    return [
        clf_params_input["n_estimators"].update(interactive=interactive),
...
def select_model(name):
    ...
    if name is None or name == "不加载模型":
        selected_model = None
        return [
            use_model_params.update(value=False, interactive=False)
        ] + clf_params_non_interactive(False)
    else:
        selected_model = name
        return [
            use_model_params.update(value=True, interactive=True)
        ] + clf_params_non_interactive(True)
...
def cv_method_change(method):
    ...
    return [
        cv_params_input["n_splits"].update(visible=visible),
...
def training(
    ...
    clf_params = {
        ...
        "ccp_alpha": clfp16,
        "max_samples": (float(clfp17) if "." in clfp17 else int(clfp17))
        if clfp17
        else None,
    }
    global selected_model
    ...
    # 处理scores
    metrics_mean = {"F1_macro平均值": scores.mean(), "标准差": scores.std()}
    metrics = {}
...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Box():
            with gr.Row():
                model_list_init()
                # 选择模型
                choice_name = gr.Dropdown(
                    choices=model_upload,
                    value="不加载模型",
                    multiselect=False,
                    label="选择模型",
                    scale=5,
                ...
                # 上传模型(*.skops)
                upload_model_btn = gr.UploadButton(
                    label="上传模型(*.skops)", scale=1, file_types=[".skops"]
                )

    with gr.Tab("训练"):
        with gr.Row():
            # 上传训练集(*.csv)
            upload_train_btn = gr.UploadButton(
                label="上传训练集(*.csv)", variant="primary", file_types=[".csv"]
            )
            train_btn = gr.Button(value="训练", variant="primary")
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    with gr.Accordion(label="分割数据集"):
                        # 数据集分割方法
                        cv_method_input = gr.Radio(
                            choices=["交叉验证法", "留一法"], value="交叉验证法", label="分割方法"
                        )
                        # 数据集分割参数
                        cv_params_input = {
                            "n_splits": 5,
                        ...
                        cv_params_input["n_splits"] = gr.Number(
                            value=cv_params_input["n_splits"],
                            label="n_splits",
                            minimum=2,
                        )
                        cv_params_input["shuffle"] = gr.Checkbox(
                            value=cv_params_input["shuffle"], label="shuffle"
                        )
                        cv_params_input["random_state"] = gr.Textbox(
                            value=cv_params_input["random_state"], label="random_state"
                        )
                        cv_params_input["p"] = gr.Number(
                            value=cv_params_input["p"], label="p", visible=False
                        )
                with gr.Row():
                    with gr.Accordion(label="训练参数"):
                        with gr.Row():
                            ...
                            use_model_params = gr.Checkbox(
                                value=False, label="使用模型参数", interactive=False
                            )
                        with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Row():
                    with gr.Accordion(label="算法准确度（F1_macro）"):
                        ...
                        with gr.Row():
                            clf_scores = gr.Label(label="子集精确度", num_top_classes=10)
                with gr.Row():
                    with gr.Accordion(label="下载模型文件"):
                        with gr.Row():
                            model_file = gr.Files(file_types=[".skops"], label="下载模型文件")
    ...
    with gr.Tab("推理"):
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    # 上传验证集(*.csv)
                    upload_validate_btn = gr.UploadButton(
                        label="上传验证集(*.csv)", file_types=[".csv"]
                    )
                    # 上传测试集(*.csv)
                    upload_test_btn = gr.UploadButton(
                        label="上传测试集(*.csv)", variant="primary", file_types=[".csv"]
                    )
                with gr.Row():
            ...
            with gr.Column():
                ...
                with gr.Row():
                    with gr.Accordion(label="下载结果json文件"):
                        with gr.Row():
                            json_file = gr.Files(
                                file_types=[".json"], label="下载结果json文件"
                            )
                with gr.Row():
    ...
    # 触发上传数据集按钮
    upload_train_btn.upload(
        fn=upload_file, inputs=upload_train_btn, outputs=train_display
    )
    upload_validate_btn.upload(
        fn=upload_file, inputs=upload_validate_btn, outputs=validate_display
    )
    upload_test_btn.upload(fn=upload_file, inputs=upload_test_btn, outputs=test_display)
    # 触发'数据集分割方法'
    cv_method_input.change(
        fn=cv_method_change,
    ...
    # 触发'使用模型参数'开关
    use_model_params.change(
        fn=clf_params_non_interactive,
    ...
    validate_btn.click(
        ...
        outputs=[validate_scores, json_file, json_display],
    )
    test_btn.click(fn=test, inputs=upload_test_btn, outputs=[json_file, json_display])
